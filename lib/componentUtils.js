...
(() => {
	...
	*/
	...
	function initDirPath(path, option = null, callback) {
	...
	function openFileWithApp(filePath, appPath, callback) {
		filePath && appPath && all([fileIsFile(filePath), fileIsExecutable(appPath)]).then(array => {
			...
			run && (
				...
				process = execFile(app, args, {}, (error, stdout, stderr) => {
					error && onError(error);
					stderr && callback && callback(stderr.toString());
				})
			);
		}, onError);
	}
