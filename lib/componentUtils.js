...
(function() {
	...
	function openFileWithApp(filePath, appPath) {
		...
		all([fileExists(filePath), fileExists(appPath)]).then(function(res) {
					...
					observe: function(subject, topic, data) {
						switch (topic) {
							case "quit-application-granted":
								subject && subject.isRunning && process.kill();
								break;
							case "process-failed":
								onError(new Error(topic));
								break;
			...
			process.init(file);
			process.runAsync([appPath], 1, ObserverHandler);
	...
	function openFilePicker(callback) {
		...
		filePicker.init(windowUtils.getMostRecentBrowserWindow(), null, nsIFilePicker.modeOpen);
