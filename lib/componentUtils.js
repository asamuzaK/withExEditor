...
(() => {
	...
	*/
	const createDir = (array, callback = null) => {
		const path = array && Array.isArray(array) && joinPath(array) || null;
		path && OS.File.makeDir(path).then(
			() => OS.File.setPermissions(path, { unixMode: 0o777 })
		).then(
			() => callback && callback(path)
	...
	*	@returns {?Function} - callback if exists
	*/
	const createFile = (array, value = "", callback = null) => {
		const path = array && Array.isArray(array) && joinPath(array) || null;
		path && OS.File.writeAtomic(
			path,
