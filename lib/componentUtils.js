...
(function() {
	...
	function forceInitDirPath(path, callback) {
		path && OS.File.removeDir(path, { ignoreAbsent: true, ignorePermissions: true }).then(function() {
			makeDirPath(path, callback ? callback : null);
		}, function() {
			notifications.notify({
				title: "Notification",
				text: "Failed to remove temporary files",
			});
		});
	}
