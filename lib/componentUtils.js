...
(function() {
	...
	*/
	...
	function openFileWithApp(filePath, appPath) {
		filePath && appPath && all([fileIsFile(filePath), fileIsExecutable(appPath)]).then(function(array) {
			for(var run, app, process, args, i = 0, l = array.length; i < l; i++) {
				run = array[i];
				if(!run) {
					break;
				}
			...
			run && (
				app = Cc["@mozilla.org/file/local;1"].createInstance(Ci.nsIFile),
	...
	*/
	...
	function compareVersion(version) {
		return Services.vc.compare(Services.appinfo.version, version ? version : DEFAULT_MIN_VERSION);
	}
