...
(function() {
	...
	var recommendMinVersion = "39.*",
		isOSX = /Darwin/.test(Services.appinfo.OS);
	...
	*/
	function fileIsExecutable(path) {
		return path ? OS.File.exists(path).then(function(bool) {
			...
			bool && (
				...
				bool = isOSX ? isAppPackage(file) : file.isExecutable()
			);
			return bool;
		}, onError) : false;
	}
	...
	function openFileWithApp(filePath, appPath) {
		filePath && appPath && all([fileIsFile(filePath), fileIsExecutable(appPath)]).then(function(array) {
			for(var run, args, app, process, i = 0, l = array.length; i < l; i++) {
				run = array[i];
				if(!run) {
					break;
				}
			...
			run && (
				args = isOSX ? ["-a", appPath, "--args", filePath] : [filePath],
				app = new FileUtils.File(isOSX ? "/usr/bin/open" : appPath),
