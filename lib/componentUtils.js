...
(function() {
	...
	function attemptRemoveDir(path) {
		OS.File.removeDir(path).then(function() {}, function() {});
	}
	...
	function fileIsExecutable(path) {
		return OS.File.exists(path).then(function(res) {
			var file;
			res && (file = new FileUtils.File(path), res = file.isExecutable());
			return res;
		}, onError);
	}
	...
	function openFileWithApp(filePath, appPath) {
		all([fileExists(filePath), fileIsExecutable(appPath)]).then(function(array) {
