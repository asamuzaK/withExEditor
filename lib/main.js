...
(() => {
  ...
  /* constants */
  const E_NOT_STRING = "Argument must be a string";
  ...
  const getFileNameFromFilePath = path =>
    isStr(path) && (path = /^([^\.]+)(?:\..+)?$/.exec(files.basename(path))) ?
      path[1] : LABEL;
  ...
  const fileIsFile = path =>
    isStr(path) && files.exists(path) && files.isFile(path);
   ...
   * @param {boolean} bool - recursive
   * @return {string} - path
   */
  const removeDir = promised((path, bool = false) => {
    if(!isStr(path)) {
      throw `${ E_NOT_STRING }. Received ${ path }.`;
    }
    files.exists(path) && fs.rmdirSync(path, bool);
    return path;
  });
   ...
   */
  const initDir = (path, bool = false, callback = null) =>
    removeDir(path, bool).then(path =>
      createDir(path)
    ).catch(e => {
      callback && callback(e);
      logError(e);
  ...
  exports.main = (option, callback) => {
    ...
    const storeTmp = simpleStorage.storage.withExEditorTmpFiles;
