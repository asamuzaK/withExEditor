...
(() => {
   ...
   * @param {Object} e - Error
   */
  const onExecFileAppError = (e, ...args) => {
   ...
   * @param {string} path - file path
   */
  const setExecFileAppParams = path => {
    if(path) {
  ...
  /**
   * handle getTmpFile Response and sync contents value
  ...
  const getSource = (url = tabs.activeTab.url,
    ...
    uri && (
      uri.protocol === "file:" ?
        setExecFileAppParams(urls.toFilename(url)) :
