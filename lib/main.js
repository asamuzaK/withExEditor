...
(() => {
   ...
   */
  const fetchFile = (url = tabs.activeTab.url,
  ...
  /**
   * on get temporary file complete
   * @param {Object} res - Response object
   */
  const getTmpFileComplete = res => {
    if(res) {
  ...
  const getTmpFile = target => {
    if(isEnabled && target) {
      ...
      const file = storeId && storeId[dirName] && storeId[dirName][target];
      file &&
        fetchFile(file.fileURI, file.type, "UTF-8", getTmpFileComplete).get();
    }
  };
  ...
  const openFileWithApp = (path, uri) => {
    ...
    isEnabled && path && uri && (
      ...
      target = getFileNameFromPath(urls.URL(uri).path),
  ...
  /**
   * on get file source complete
   * @param {Object} res - Response object
   */
  const getSourceComplete = res => {
    if(res) {
    ...
    if(isEnabled && urls.isValidURI(url)) {
      ...
      if(uri.protocol === "file:") {
        try {
          ...
          fetchFile(url, type, charset, getSourceComplete).get();
        }
      }
      else {
        fetchFile(url, type, charset, getSourceComplete).get();
