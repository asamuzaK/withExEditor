...
"use strict";
{
  ...
  class StorageTmp {
    ...
    /**
     * get property value
     * @param {{dir:string, id:string, host:string, target:string}} obj
     *        - descendant's property
     * @return {*}
     */
    get(obj) {
      const dir = obj && isString(obj.dir) && obj.dir;
      const id = obj && isString(obj.id) && obj.id;
      const host = obj && isString(obj.host) && obj.host;
      const target = obj && isString(obj.target) && obj.target;
      const store = internal(this);
      return obj && dir && id && host && target &&
             store.storage[dir] && store.storage[dir][id] &&
             store.storage[dir][id][host] &&
               store.storage[dir][id][host][target];
    }
    ...
    set(obj = {}, val = null) {
      ...
      const store = internal(this);
      dir && (store.storage[dir] = store.storage[dir] || id && {} || val) &&
      id && (store.storage[dir][id] = store.storage[dir][id] ||
                                      host && {} || val) &&
      host && (store.storage[dir][id][host] = store.storage[dir][id][host] ||
                                              target && {} || val) &&
      target && (store.storage[dir][id][host][target] = val);
    }
    ...
    remove(obj = {}) {
      ...
      const target = isString(obj.target) && obj.target;
      const store = internal(this);
      return dir && !id && store.storage[dir] ?
               delete internal(this).storage[dir] :
             dir && id && !host &&
             store.storage[dir] && store.storage[dir][id] ?
               delete internal(this).storage[dir][id] :
             dir && id && host && !target && store.storage[dir] &&
             store.storage[dir][id] && store.storage[dir][id][host] ?
               delete internal(this).storage[dir][id][host] :
             dir && id && host && target && store.storage[dir] &&
             store.storage[dir][id] && store.storage[dir][id][host] &&
             store.storage[dir][id][host][target] &&
               delete internal(this).storage[dir][id][host][target];
    }
  }
