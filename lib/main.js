...
"use strict";
{
   ...
   * @return {void}
   */
  const createFile = (path, value = "", callback = null, opt = null,
                      encoding = CHAR) => {
    isString(path) &&
   ...
   */
  const spawnProcess = async (path, tabId) => {
   ...
   */
  const createTmpFileFromRes = (res, data) => {
    const tab = tabs.activeTab;
  ...
  const getSource = (url = tabs.activeTab.url,
    ...
    return uri && (
             ...
             createRequest(uri, type, charset, createTmpFileFromRes, data).get()
           );
  };
    ...
    switch (mode) {
      case MODE_EDIT_TEXT:
        return (target = data.target) &&
                 ...
                 getSource(url, type, charset, data);
      case MODE_MATHML:
      ...
      case MODE_SVG:
        return value && (target = getFileNameFromURI(url.path, "index")) &&
                 ...
                 getSource(url, type, charset, data);
      default:
        return getSource(url, type, charset, data);
    }
  };
