...
(() => {
  ...
  */
  const onStorageOverQuota = evt => {
    ...
    evt && evt.type && evt.type === "OverQuota" && storageId && (
      storageId[isPb ? PB_TMP_DIR : TMP_DIR] = {},
  ...
  */
  const getTmpFile = target => {
    let dirName, file;
    isEnabled && target && (
      dirName = isPb ? PB_TMP_DIR : TMP_DIR,
  ...
  */
  const setProcessIdSubDir = path => {
    path && (
      ...
      comUtils.createDir(paths.join(tmpDir, PB_TMP_DIR)),
