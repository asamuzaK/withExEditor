...
(() => {
	...
	exports.main = (option, callback) => {
		...
		function getFileNameFromUriPath(path, value) {
			var fileName = path && /(?:.*\/)?((?:[a-zA-Z0-9\-_~!\$&'\(\)\*\+,;=:@]|%[0-9a-fA-F]{2})+)(?:\.(?:[a-zA-Z0-9\-\_~!\$&'\(\)\*\+,;=:@]|%[0-9a-fA-F]{2})+)*$/.exec(path);
		...
		function onLastPbContextExisted() {
			var path, opt = prefsForceRemove ? { ignorePermissions: true } : null;
	...
	exports.onUnload = reason => {
		var path, opt = simplePrefs.prefs[PREFS_FORCE_REMOVE] ? { ignorePermissions: true } : null;
		simpleStorage.storage.withExEditorTmpFiles && processId && (
