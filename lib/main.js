...
"use strict";
{
  ...
  class TemporaryStorage {
     ...
     * @param {{dir:string, tId:string, host:string, target:string}} key
     *        - descendant property keys
     * @return {*}
     */
    get(key) {
      const store = internal(this).storage;
      const dir = key && isString(key.dir) && key.dir;
      const tId = key && isString(key.tId) && key.tId;
      const host = key && isString(key.host) && key.host;
      const target = key && isString(key.target) && key.target;
    ...
    /**
     * set property / value
     * @param {Object} key - descendant property keys
     * @param {*} val - value
     * @return {void}
     */
    set(key = {}, val = null) {
      const store = internal(this).storage;
      const dir = isString(key.dir) && key.dir;
      const tId = isString(key.tId) && key.tId;
      const host = isString(key.host) && key.host;
      const target = isString(key.target) && key.target;
  ...
  const createTmpFile = (res = {}, value = "") => {
    ...
    return tId && host && target && file &&
             createDir(files.join(TMP_DIR, dir, tId, host)).then(path => {
               storage.set({dir, tId, host, target}, res);
  ...
  class AccessKey {
     ...
     */
    constructor(key) {
      internal(this).key = isString(key) && key.length === 1 && key || "";
    }
    get key() {
      return internal(this).key;
    }
    set key(key) {
      isString(key) && key.length === 1 && (internal(this).key = key);
    }
  }
  ...
  exports.main = () => {
    ...
    // TODO: remove store.toolbarButtonIcon at future release
    (store.buttonIcon || store.toolbarButtonIcon) &&
      (buttonIcon = store.buttonIcon || store.toolbarButtonIcon);
