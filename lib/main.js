...
"use strict";
{
   ...
   * @return {boolean}
   */
  const isEnabled = () => !isPb() || prefs.enablePB;
  ...
  const removePrivateTmpFiles = evt =>
    evt && evt.type === "last-pb-context-exited" &&
      removeDir(
  ...
  class ChildProcess {
    ...
    spawn(file) {
      return Promise.all([isFile(file), isExecutable(this.app)]).then(arr => {
        ...
        return proc || notifyWarn(_("FailSpawn"));
      }).catch(logError);
    }
  }
  ...
  const spawnProcess = path => {
    ...
    return path && prefs.editorPath &&
           tab.once("close", removeTabRelatedStorage) &&
             (new ChildProcess({
               app: prefs.editorPath,
               cmd: prefs.editorCmdArgs,
               env: getSystemEnv(),
               pos: prefs.editorCmdPos,
               shell: prefs.editorShell
    ...
    isString(key) && key.length === 1 &&
      new PageMod({
        ...
        contentScriptOptions: {
          ...
          metaKey: isMac(),
          shiftKey: true,
          onlyEdit: prefs.editableContext,
          data: nsURI
        },
  ...
  class AccessKey {
     ...
     * @param {boolean} [opt.hotkey] - enable hot key
     * @param {Function} [opt.onPress] - callback on hot key press
  ...
  /* context menu item */
  const menu = new Item({
    label: _(VIEW_SOURCE, prefs.editorName),
    image: self.data.url(`${ICON}#gray`),
    contentScriptFile: JS_MENU,
    context: [
      enableContext,
      prefs.editableContext && selectorContext || mediaTypeContext
    ],
    accesskey: accKey.key || null,
    data: nsURI,
    onMessage: res => {
      /^(?:View(?:MathML|S(?:ource|election))|EditText)$/.test(res) ?
        menu.label = _(res, prefs.editorName) :
  ...
  exports.main = () => {
    webExtension.startup().then(api => {
      ...
      const SET_SDK_PREFS = "setSdkPrefs";
      ...
      const portWebExtension = async port => {
        ...
        const checkEnabled = async () => {
          const toggleEnabled = await isEnabled();
        ...
        const getSdkPrefs = async () => {
          ...
          const sdkPrefs = simplePrefs.prefs;
          const editorPath = sdkPrefs && sdkPrefs[`${PREFS}File`] || "";
          const editorName = editorPath && sdkStorage.editorName || "";
          const editorCmdArgs = sdkPrefs &&
                                  sdkPrefs[`${PREFS}CmdArgs`] || "";
          const editorCmdPos = sdkPrefs && sdkPrefs[`${PREFS}CmdPos`] || false;
          const editorShell = sdkPrefs && sdkPrefs[`${PREFS}Shell`] || false;
          const accessKey = sdkPrefs && sdkPrefs[`${PREFS}AccessKey`] || "e";
          const optionsShortCut = sdkPrefs &&
                                    sdkPrefs[`${PREFS}Hotkey`] || true;
          const editorShortCut = sdkPrefs && sdkPrefs[`${PREFS}ExecEditor`] ||
                                   true;
          const enablePB = sdkPrefs && sdkPrefs[`${PREFS}EnablePB`] || false;
          const editableContext = sdkPrefs && sdkPrefs[`${PREFS}Context`] ||
                                    false;
          const forceRemove = sdkPrefs && sdkPrefs[`${PREFS}ForceRemove`] ||
                                true;
          const resSdkPrefs = {
        ...
        const checkExecutable = async path => {
          ...
          const name = executable && await getFileNameFromFilePath(path) || "";
          return {
            resExecutable: {
              executable, name,
              value: path
        ...
        /* add listeners */
        browserWindows.on("activate", () => {
          (new urls.URL(tabs.activeTab.url)).scheme !== "moz-extension" &&
            checkEnabled().then(portMsg).catch(logError);
        });
        ...
        ]).then(checkEnabled).then(portMsg).catch(logError);
      };
  ...
  exports.onUnload = reason => {
    ...
    const sdkPrefs = simplePrefs.prefs;
    sdkPrefs && (
      sdkPrefs[`${PREFS}File`] && delete sdkPrefs[`${PREFS}File`],
      sdkPrefs[`${PREFS}Path`] && delete sdkPrefs[`${PREFS}Path`],
      sdkPrefs[`${PREFS}CmdArgs`] && delete sdkPrefs[`${PREFS}CmdArgs`],
      sdkPrefs[`${PREFS}CmdPos`] && delete sdkPrefs[`${PREFS}CmdPos`],
      sdkPrefs[`${PREFS}Shell`] && delete sdkPrefs[`${PREFS}Shell`],
      sdkPrefs[`${PREFS}AccessKey`] && delete sdkPrefs[`${PREFS}AccessKey`],
      sdkPrefs[`${PREFS}Hotkey`] && delete sdkPrefs[`${PREFS}Hotkey`],
      sdkPrefs[`${PREFS}ExecEditor`] && delete sdkPrefs[`${PREFS}ExecEditor`],
      sdkPrefs[`${PREFS}EnablePB`] && delete sdkPrefs[`${PREFS}EnablePB`],
      sdkPrefs[`${PREFS}Context`] && delete sdkPrefs[`${PREFS}Context`],
      sdkPrefs[`${PREFS}ForceRemove`] && delete sdkPrefs[`${PREFS}ForceRemove`],
      sdkPrefs[`${PREFS}Panel`] && delete sdkPrefs[`${PREFS}Panel`]
    );
    simpleStorage.storage.withExEditor &&
      delete simpleStorage.storage.withExEditor;

    /* remove temporary files */
    removeDir(TMP_DIR, prefs.forceRemove).catch(logError);
