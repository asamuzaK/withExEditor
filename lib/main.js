...
"use strict";
{
  ...
  const getSystemEnv = () => {
    const env = {};
    [
      "USER", "HOME", "DISPLAY", "XAUTHORITY", "LANG", "PATH",
      "USERNAME", "HOMEDRIVE", "HOMEPATH", "USERPROFILE", "APPDATA"
    ].forEach(vars => {
  ...
  class ChildProcess {
     ...
     * @param {string} [prop.app] - application path
     * @param {string} [prop.cmd] - command line options
     * @param {Object} [prop.env] - environment variables
     * @param {boolean} [prop.shell] - run command within a shell
     * @return {void}
     */
    set(prop = {}) {
      this.app = isExecutable(prop.app) && prop.app || "";
      this.cmd = isString(prop.cmd) && prop.cmd || "";
      this.opt = prop.env && Object.keys(prop.env).length > 0 &&
                   {env: prop.env} || null;
      this.shell = !!prop.shell;
    }
     ...
     */
    spawn(file) {
      const proc = isFile(file) && !!this.app &&
                     spawn(
                       this.shell ?
      ...
      proc && (
        ...
        proc.on("close", code => {
          code !== 0 && notifyWarn(`Process exited with code ${code}`);
        }),
  ...
  const spawnProcess = path => {
    ...
    return path && tab.once("close", removeTabRelatedStorage) &&
             proc.spawn(path) ||
