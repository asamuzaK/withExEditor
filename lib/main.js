...
(() => {
  ...
  let prefsFile, prefsArgs, prefsDisp, prefsKey, prefsPb, prefsRemove,
      storeId, tmpDir, isPb, isEnabled, editorName = "", editorPath = "",
  ...
  /**
   * handle child process callback
   * @param {Error} error
   * @param {Buffer} stdout
   * @param {Buffer} stderr
   */
  const onChildProcessCallback = (error, stdout, stderr) => {
    switch(true) {
      case error:
        logError(error);
        notifyError(error);
   ...
   */
  const getFileNameFromPath = (path, value = LABEL) =>
    path && (
  ...
  /**
   * handle Response of getTmpFile()
   * @param {Object} res - Response object
   */
  const onTmpFileResponse = res => {
    if(res) {
      const value = res.text || "";
      const target = getFileNameFromPath(urls.URL(res.url).path);
  ...
  const openFileWithApp = (path, uri) => {
    /**
     * check file and editor meets there requirements
     * @param {string} file - file path
     * @param {string} editor - editor path
     * @return {boolean}
     */
    const checkFileRequirements = (file, editor) =>
      file && editor ? all([
        comUtils.fileIsFile(file),
        comUtils.fileIsExecutable(editor)
      ]).then(arr => {
        let bool;
        for(let res of arr) {
          bool = res;
    ...
    if(isEnabled && path && urls.isValidURI(uri) && editorPath) {
      const dirName = isPb ? TMP_DIR_PB : TMP_DIR;
      const target = getFileNameFromPath(urls.URL(uri).path);
      storeId && storeId[dirName] && storeId[dirName][target] &&
        (storeId[dirName][target].fileURI = uri);
      checkFileRequirements(path, editorPath) && execFile(
        editorPath,
        /\S+/.test(prefsArgs) ?
          [path].concat(
            prefsArgs.match(/"[^"]?(?:(?:[^"]|\\"[^"]?(?:[^"]*[^"\s])?\\")*[^"\s])?"|[^"\s]+/g)
          ) : [path],
        prefsDisp ? { env: { DISPLAY: prefsDisp } } : {},
        onChildProcessCallback
      );
    }
  };
  ...
  /**
   * handle Response of getSource()
   * @param {Object} res - Response object
   */
  const onSourceResponse = res => {
    if(res) {
      const target = getFileNameFromPath(urls.URL(res.url).path, "index");
      let type = /^([^\/]+\/[^;]+)(?:;\s*.+)?$/.exec(res.headers["Content-Type"]);
      (type = type && type[1]) && target && setTmpFile({
        mode: VIEW_SOURCE,
      ...
      if(uri.protocol === "file:") {
        ...
        path ? (
          storeTmpFile(
            {
              mode: VIEW_SOURCE,
              target: getFileNameFromPath(uri.path, "index")
            },
            isPb ? TMP_DIR_PB : TMP_DIR
          ),
          openFileWithApp(path, uri.toString())
        ) : createRequest(url, type, charset, onSourceResponse).get();
      }
      else {
        createRequest(url, type, charset, onSourceResponse).get();
      }
  ...
  const getContextMode = (res = `{ "mode": "${ VIEW_SOURCE }" }`) => {
    ...
    switch(res.mode) {
      ...
      case VIEW_SELECTION:
        value ? (
          target = getFileNameFromPath(urls.URL(url).path, "index"),
   ...
   * @return {void}
   */
  const setPrefSubDir = path => {
    path && (
      tmpDir = path,
      comUtils.createDir(paths.join(tmpDir, TMP_DIR)),
      comUtils.createDir(paths.join(tmpDir, TMP_DIR_PB)),
      storeId = storePrefTmpFiles(simpleStorage.storage.withExEditorTmpFiles)
    );
    return;
  };

  /**
   * set preference ID named directory in temporary directory
   * @param {string} path - directory path
   * @return {void}
   */
  const setPrefTmpDir = path => {
    path && comUtils.createDir(paths.join(path, PREF_ID), setPrefSubDir);
    return;
  };
   ...
   */
  const togglePanel = () => {
    panel && (panel.isShowing ? panel.hide() : showPanel());
    return;
  };
  ...
  const replaceEditor = bool => {
    bool ? (
      ...
      editorName = prefsFile ?
        getFileNameFromPath(urls.URL(urls.fromFilename(editorPath)).path) : ""
    ) : (
      editorPath = "",
      editorName = "",
      menuItem = null,
  ...
  exports.main = (option, callback) => {
    ...
    const setItems = (bool = false) => {
      ...
      bool && prefsFile && (
        ...
        editorName = stored && stored.editorPath &&
                     ...
                     stored.editorName ?
                       stored.editorName :
                       getFileNameFromPath(
                         urls.URL(urls.fromFilename(prefsFile)).path
                       )
      );
      ...
      comUtils.createDir(
        paths.join(system.pathFor("TmpD"), LABEL),
        setPrefTmpDir
      );
  ...
  exports.onUnload = reason => {
    ...
    reason === "disable" && simplePrefs.prefs[`${ PREFS }OnDisable`] && (
