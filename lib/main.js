...
(function() {
	...
	exports.main = function(option, callback) {
		var storageTmpFilesProcessId,
			...
			unixMode = 0o666;	// default should be 0o600. set on pref ? controlPanel ?
		...
		function notifyOnSpawnStderr(e) {
		...
		function openFileWithExEditor(filePath, fileURI) {
			...
			processId && isEnabled && filePath && fileURI && (
				...
				target && storageTmpFilesProcessId && storageTmpFilesProcessId[tmpFiles] && storageTmpFilesProcessId[tmpFiles][target] && (
					...
					componentUtils.openFileWithApp(filePath, simplePrefs.prefs[PREFS_FILE_NAME], notifyOnSpawnStderr)
				)
			);
		}
		function setTmpFile(res) {
			...
			res && isEnabled && (
				...
				componentUtils.createFile([processIdTmpDir, tmpFiles, res["fileName"]], unixMode, res["value"], openFileWithExEditor)
			);
		}
		function getSource() {
		...
		function onLastPbContextExisted() {
			...
			processIdTmpDir && storageTmpFilesProcessId && (
				...
				componentUtils.initDirPath(path, option, notifyOnFailRemoveTmpFiles),
