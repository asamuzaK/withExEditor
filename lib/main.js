...
(() => {
   ...
   * @return {Object} - Promise object which returns child process object
   */
  ...
  const setTmpDir = () => {
    /**
     * set store preference ID temporary files data
    ...
    comUtils.createDir(paths.join(system.pathFor("TmpD"), LABEL)).then(path =>
      ...
      arr && setStoreId(simpleStorage.storage.withExEditorTmpFiles)
    ).catch(logError);
  };
