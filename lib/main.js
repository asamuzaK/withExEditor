...
"use strict";
{
  ...
  const isExecutable = async (path, mask = MASK_BIT) =>
    await isFile(path) && !!((new fs.Stats(path)).mode & mask);
   ...
   * @return {void}
   */
  const replaceEditor = async pref => {
    const path = prefs[pref];
    if (pref === `${PREFS}File`) {
      await isExecutable(path).catch(logError) ? (
        editorPath = path,
      ...
      pref === `${PREFS}Path` && await isExecutable(path).catch(logError) &&
   ...
   * @return {void}
   */
  const setVariablesFromStorage = async () => {
  ...
  exports.main = () => {
    /* create temporary directories, set components */
    Promise.all([
      createDir(join(TMP_DIR, TMP_FILES)),
      createDir(join(TMP_DIR, TMP_FILES_PB)),
      setVariablesFromStorage().then(setToolbarButton).
        then(storeAll({editorName, editorPath, buttonIcon})),
      setEventListeners()
