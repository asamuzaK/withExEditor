...
(function() {
	...
	var processId = LABEL + (new Date().getTime());
	...
	exports.main = function(option, callback) {
		...
		const PREFS_NOTIFY_INIT = "withExEditorNotifyInitPB";
		...
		function storeProcessIdTmpFiles() {
			processId && processIdTmpDir && (simpleStorage.storage.withExEditorTmpFiles[processId] = {
				"processId": processId,
				"tmpDir": tmpDir,
		...
		/* remove tmp files at the end of Private Browsing */
		function onLastPbContextExisted() {
			var path;
			processId && processIdTmpDir && (
				path = componentUtils.joinPath([processIdTmpDir, PB_TMP_FILES_DIR_NAME]),
				prefsNotifyInitPb ? componentUtils.forceInitDirPath(path) : componentUtils.initDirPath(path),
				simpleStorage.storage.withExEditorTmpFiles[processId]["pbTmpFiles"] = {}
			);
		}
		...
		function openFileWithExEditor(filePath, fileURI) {
			var isPb = isPrivate(browserWindows.activeWindow),
				bool = !isPb || isPb && prefsEnablePb,
				tmpFiles, target;
			processId && bool && filePath && fileURI && (
		...
		function setContextMenuItem() {
			contextMenuItem = contextMenu.Item({
				...
				context: contextMenu.PredicateContext(function(context) {
					var isPb = isPrivate(browserWindows.activeWindow),
						bool = !isPb || isPb && prefsEnablePb;
					return bool && /^(?:application\/(?:(?:[a-zA-Z0-9\.\-_]+\+)?(?:json|xml)|(?:ecm|jav)ascript)|image\/[a-zA-Z0-9\.\-_]+\+xml|text\/[a-zA-Z0-9\.\-_]+)$/.test(context.documentType);
				}),
			...
			if(prefName) {
				switch(prefName) {
					...
					case PREFS_ENABLE_PB:
						...
						break;
					case PREFS_NOTIFY_INIT:
						prefsNotifyInitPb = simplePrefs.prefs[prefName];
						break;
					default:
				}
			}
		...
		function setItems(bool) {
			...
			simplePrefs.on(PREFS_NOTIFY_INIT, onPrefsChange);
		...
		(function getPrefsFile() {
			...
			prefsNotifyInitPb = simplePrefs.prefs[PREFS_NOTIFY_INIT] || false;
			...
			browserWindows.on("activate", function(win) {
				var isPb = isPrivate(win),
					bool = !isPb || isPb && prefsEnablePb;
				toolbarButton && (
					bool ? (
