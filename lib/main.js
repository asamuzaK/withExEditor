...
"use strict";
{
  ...
  const EXP_FILE_EXT = /^(application|image|text)\/([\w\-\.]+)(?:\+(json|xml))?$/;
  const EXP_FILE_PATH = /^.*\/((?:[\w\-~!\$&'\(\)\*\+,;=:@]|%[0-9A-F]{2})+)(?:(?:\.(?:[\w\-~!\$&'\(\)\*\+,;=:@]|%[0-9A-F]{2})+)*(?:\?(?:[\w\-\.~!\$&'\(\)\*\+,;=:@\/\?]|%[0-9A-F]{2})*)?(?:#(?:[\w\-\.~!\$&'\(\)\*\+,;=:@\/\?]|%[0-9A-F]{2})*)?)?$/;
  ...
  const tmpDir = files.join(
    system.pathFor("TmpD"),
    LABEL,
    /^.+\.([^\.]+)$/.exec(files.basename(system.pathFor("PrefD")))[1] || LABEL
  );
  ...
  const getFileExtension = (mime = "text/plain", fallback = "txt") => {
    let ext = EXP_FILE_EXT.exec(mime);
    if (ext) {
      const type = ext[1];
      const subtype = ext[2];
  ...
  const getTmpFile = target => {
    ...
    const file = target && storageTmp.get({
      dir: isPb() ? TMP_DIR_PB : TMP_DIR,
      id: tab.id,
      host: (new urls.URL(tab.url)).host || LABEL,
      target
    });
    return file && createRequest(file.uri, file.type, "UTF-8",
                                 attachTab, target).get();
  };
  ...
  const getSource = (url = tabs.activeTab.url,
    ...
    return uri && (
             uri.scheme === "file" ?
               spawnProcess(urls.toFilename(uri)) :
