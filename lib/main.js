...
"use strict";
{
   ...
   * @return {Object} - Promise.<boolean> - result
   */
  ...
  /**
   * temporary files data storage class
   */
  class TempDataStorage {
     ...
     */
    constructor() {
      internal(this).storage = {[TMP_FILES]: {}, [TMP_FILES_PB]: {}};
    }
    ...
    get(keys = {}) {
      ...
      const {dir, tabId, host, target} = keys;
  ...
  /* temporary files data storage */
  const tmpDataStorage = new TempDataStorage();

  /**
   * remove temporary file data on tab close
  ...
  const removeTabRelatedStorage = async data => {
    let bool;
    if (data) {
      const {incognito, tabId} = data;
      const dir = incognito && TMP_FILES_PB || TMP_FILES;
      bool = tabId && tmpDataStorage.remove({dir, tabId});
    }
    return bool || false;
  };
  ...
  const removePrivateTmpFiles = () =>
    removeDir(DIR_TMP_FILES_PB, vars[FORCE_REMOVE]).then(() =>
      ...
      tmpDataStorage.remove({dir: TMP_FILES_PB});
      tmpDataStorage.set({dir: TMP_FILES_PB}, {});
    });
  ...
  const storeTmpFilePath = async (path, data) => {
    if (path && data) {
      const {host, incognito, tabId, target} = data;
      const dir = incognito && TMP_FILES_PB || TMP_FILES;
      dir && tabId && host && target && (
        data.path = path,
        tmpDataStorage.set({dir, tabId, host, target}, data)
      );
    }
  };
  ...
  const getTmpFileData = async data => {
    ...
    if (data) {
      const {host, incognito, tabId, target} = data;
      const dir = incognito && TMP_FILES_PB || TMP_FILES;
      store = tmpDataStorage.get({dir, tabId, host, target});
      store && store.path && (
        store.timestamp = await getFileTimestamp(store.path),
        tmpDataStorage.set({dir, tabId, host, target}, store)
      );
    }
    return store || null;
  };
  ...
  const createTmpFile = async (data = {}, value = "", callback = null) => {
    const {fileName, host, incognito, tabId} = data;
    const dir = incognito && TMP_FILES_PB || TMP_FILES;
    const path = tabId && host && join(DIR_TMP, dir, tabId, host);
    ...
    webExtension.startup().then(api => {
      ...
      const portWebExtension = async port => {
        ...
        const portSyncText = async (value, data) => {
          let msg;
          if (value && data) {
            const {target, tabId} = data;
            msg = {
              [SYNC_TEXT]: {data, tabId, target, value},
            };
          }
            ...
            for (const item of items) {
              ...
              switch (item) {
                ...
                case GET_FILE_PATH:
                  ...
                  break;
                case GET_TMP_FILE:
                  func.push(getTmpFileData(obj).then(getTmpFile));
                  break;
                ...
                case REMOVE_TAB_STORAGE:
                  func.push(removeTabRelatedStorage(obj));
                  break;
                case FORCE_REMOVE:
                  vars[item] = !!obj;
                  break;
