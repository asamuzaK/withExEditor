...
(() => {
  ...
  */
  const onStorageOverQuota = evt => {
    let msg;
    evt && evt.type && evt.type === OVER_QUOTA && storageId && (
  ...
  */
  const setToolbarButton = stored => {
    ...
    switch(true) {
      case editorName === "":
        button.badge = "!";
        button.badgeColor = "#c13832";
        break;
      case !stored && editorName !== "":
        button.badge = "i";
        button.badgeColor = "#00539f";
        break;
      default:
        button.badge = null;
        button.badgeColor = null;
  ...
  const onPrefsChange = (prefName = "") => {
    switch(prefName) {
      case `${ PREFS }File`:
        ...
        break;
      case `${ PREFS }CmdArgs`:
        prefsArgs = simplePrefs.prefs[prefName];
        break;
      case `${ PREFS }EnvDisplay`:
        prefsDisp = simplePrefs.prefs[prefName];
        break;
      case `${ PREFS }AccessKey`:
        prefsKey = checkAccessKey(simplePrefs.prefs[prefName]);
        prefsKey !== "" && (
        ...
        break;
      case `${ PREFS }EnablePB`:
        prefsPb = simplePrefs.prefs[prefName];
        onActiveWindow(browserWindows.activeWindow);
        break;
      case `${ PREFS }ForceRemove`:
        prefsRemove = simplePrefs.prefs[prefName];
        break;
      default:
    }
  };
