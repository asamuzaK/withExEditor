...
(() => {
	...
	*  @returns { string }
	*/
	const getFileNameFromUriPath = (path, value) => {
	...
	*/
	...
	/* worker port emit */
	const workerPortEmit = (worker, evt, value) => worker && evt && value && worker.port.emit(evt, value);

	/* simpleStorage */
	const storeProcessIdTmpFiles = () => {
		simpleStorage.storage.withExEditorTmpFiles && (
			...
			storageProcessId = simpleStorage.storage.withExEditorTmpFiles[PROCESS_ID]
		);
	};
	...
	const storeSettings = store => store && (simpleStorage.storage.withExEditor = store);
	const storeKeyValue = (key, value = "") => key && (simpleStorage.storage.withExEditor[key] = value);
	...
	const openFileWithExEditor = (filePath, fileURI) => {
		let tmpFiles, target;
	...
	const onLastPbContextExisted = () => {
		...
		processIdTmpDir && storageProcessId && (
			...
			componentUtils.initDirPath(path, prefsForceRemove ? { ignorePermissions: true } : null, notifyOnError),
	...
	const checkAccessKey = k => k && k.length === 1 ? k : "";
	...
	exports.onUnload = reason => {
		...
		storageProcessId && storageProcessId["processIdTmpDir"] && (
			componentUtils.attemptRemoveDir(storageProcessId["processIdTmpDir"], simplePrefs.prefs[PREFS_FORCE_REMOVE] ? { ignorePermissions: true } : null),
