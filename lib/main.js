...
"use strict";
{
  ...
  const correctArg = (arg = "") => {
    /^\s*(?:".*"|'.*')\s*$/.test(arg) ? (
      ...
      /".*"|'.*'/.test(arg) && (
        arg = arg.replace(/"([^"]+)*"|'([^']+)*'/g, (m, p1, p2) => p1 || p2)
      )
    )
    return arg;
  };
   ...
   */
  const getArgArr = (path = "", cmd = "") => {
    const args = cmd.match(EXP_CMD);
    return args && [path].concat(args.map(correctArg)) || [path];
  };
  ...
  class ChildProcess {
    ...
    spawn(file) {
      return isFile(file) && isExecutable(this.app) &&
               execFile(this.app, getArgArr(file, this.cmd), this.opt, this.cb);
    }
  }
