...
(() => {
  ...
  /* component utilities */
  const compoUtils = require("./componentUtils");
  ...
  const PROCESS_ID = compoUtils.getProfName() || LABEL;
  ...
  */
  const storeTmpFile = (store, dirName) => {
    store && store.target && isStr(dirName) && storageId && storageId[dirName] &&
  ...
  */
  const storeKeyValue = (key, value = "") => {
    isStr(key) && (simpleStorage.storage.withExEditor[key] = value);
    return;
  };
  ...
  */
  const openFileWithApp = (filePath, fileURI) => {
    let dirName, target;
    isEnabled && isStr(filePath) && isStr(fileURI) && (
      dirName = isPb ? PB_TMP_FILES_DIR_NAME : TMP_FILES_DIR_NAME,
      ...
      editorPath !== "" && all([
        compoUtils.fileIsFile(filePath),
        compoUtils.fileIsExecutable(editorPath)
      ]).then(
        array => {
          let run;
          for(let i of array) {
  ...
  */
  const setProcessIdSubDir = path => {
    isStr(path) && (
      tmpDir = path,
      compoUtils.createDir(paths.join(tmpDir, TMP_FILES_DIR_NAME)),
      compoUtils.createDir(paths.join(tmpDir, PB_TMP_FILES_DIR_NAME)),
  ...
  */
  const setProcessIdTmpDir = path => {
    isStr(path) &&
      compoUtils.createDir(paths.join(path, PROCESS_ID), setProcessIdSubDir);
    return;
  };
  ...
  const setControlPanel = () => {
    ...
    controlPanel.port.on("load", () => {
      controlPanel.port.emit("htmlValue", {
        ...
        compat: compoUtils.compareVersion(RECOMMEND_MIN_VERSION),
    ...
    if(isStr(prefName)) {
      switch(prefName) {
        case PREFS_APP_FILE:
          ...
          compoUtils.fileIsExecutable(prefsFile).then(replaceEditor, logError);
          break;
        case PREFS_CMD_ARGS:
  ...
  exports.main = (option, callback) => {
    ...
    */
    const setItems = (bool = false) => {
      ...
      compoUtils.createDir(paths.join(tmpDPath, LABEL), setProcessIdTmpDir);
    ...
    prefsFile ?
      compoUtils.fileIsExecutable(prefsFile).then(setItems, logError) :
      setItems(false);
  };
