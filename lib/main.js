...
(() => {
  ...
  */
  const getFileNameFromUriPath = (path, value = LABEL) =>
    path && (
      path = /^.*\/((?:[a-zA-Z0-9\-_~!\$&'\(\)\*\+,;=:@]|%[0-9a-fA-F]{2})+)(?:(?:\.(?:[a-zA-Z0-9\-\_~!\$&'\(\)\*\+,;=:@]|%[0-9a-fA-F]{2})+)*(?:\?(?:[a-zA-Z0-9\-\._~!\$&'\(\)\*\+,;=:@\/\?]|%[0-9A-F]{2})*)?(?:#(?:[a-zA-Z0-9\-\._~!\$&'\(\)\*\+,;=:@\/\?]|%[0-9A-F]{2})*)?)?$/.exec(path)
    ) ? path[1] : value;
  /**
  ...
  const setContextMenuItem = () => {
    ...
    contextMenuItem = new Item({
      ...
      "context": contextMenu.PredicateContext(
        context =>
          isEnabled &&
          /^(?:application\/(?:(?:[a-zA-Z0-9\.\-_]+\+)?(?:json|xml)|(?:ecm|jav)ascript)|image\/[a-zA-Z0-9\.\-_]+\+xml|text\/[a-zA-Z0-9\.\-_]+)$/.test(context.documentType)
      ),
  ...
  * @param {string} k - key
  * @return {string} - a single character key or empty string
  */
  ...
  */
  ...
  exports.main = (option, callback) => {
    ...
    */
    ...
    prefsFile ?
      componentUtils.fileIsExecutable(prefsFile).then(setItems, logOnError) :
      setItems(false);
  };
