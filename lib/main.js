...
"use strict";
{
  ...
  const expressionContext = new contextMenu.PredicateContext(context =>
  ...
  const menu = new contextMenu.Item({
    ...
    context: [
      enableContext,
      prefs[`${PREFS}Context`] && selectorContext || expressionContext
    ],
  ...
  const replaceContext = pref => {
    ...
    bool ? (
      menu.context.remove(expressionContext),
      menu.context.add(selectorContext)
    ) : (
      menu.context.remove(selectorContext),
      menu.context.add(expressionContext)
    );
  };
