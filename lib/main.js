...
"use strict";
{
  ...
  class ChildProcess {
    ...
    spawn(file) {
      return Promise.all([isFile(file), isExecutable(this.app)]).then(arr => {
        ...
        return proc || notifyWarn(_("FailSpawn", prefs.editorName));
      }).catch(logError);
    }
  }
  ...
  const spawnProcess = path => {
    ...
    return path && prefs.editorPath &&
           tab.once("close", removeTabRelatedStorage) &&
             ...
             })).spawn(path) || notifyWarn(_("FailSpawn", prefs.editorName));
  };
      ...
      new PageMod({
        ...
        onAttach: worker => {
          ...
          worker.port.on("pageContent", res => {
            prefs.isEnabled && !!prefs.editorPath &&
      ...
      webExtension.startup().then(api => {
        ...
        const portWebExtension = async port => {
          ...
          const getSdkPrefs = async () => {
            ...
            const editorPath = sdkPrefs[`${PREFS}File`] || "";
            const editorName = editorPath && sdkStorage.editorName || "";
            const editorCmdArgs = sdkPrefs[`${PREFS}CmdArgs`] || "";
            const editorCmdPos = sdkPrefs[`${PREFS}CmdPos`] || false;
            const editorShell = sdkPrefs[`${PREFS}Shell`] || false;
            const accessKey = sdkPrefs[`${PREFS}AccessKey`] || "e";
            const optionsShortCut = sdkPrefs[`${PREFS}Hotkey`] || true;
            const editorShortCut = sdkPrefs[`${PREFS}ExecEditor`] || true;
            const enablePB = sdkPrefs[`${PREFS}EnablePB`] || false;
            const editableContext = sdkPrefs[`${PREFS}Context`] || false;
            const forceRemove = sdkPrefs[`${PREFS}ForceRemove`] || true;
            const resSdkPrefs = {
  ...
  exports.onUnload = reason => {
    ...
    /* remove sdk prefs */
    const sdkPrefs = simplePrefs.prefs;
    delete sdkPrefs[`${PREFS}File`];
    delete sdkPrefs[`${PREFS}Path`];
    delete sdkPrefs[`${PREFS}CmdArgs`];
    delete sdkPrefs[`${PREFS}CmdPos`];
    delete sdkPrefs[`${PREFS}Shell`];
    delete sdkPrefs[`${PREFS}AccessKey`];
    delete sdkPrefs[`${PREFS}Hotkey`];
    delete sdkPrefs[`${PREFS}ExecEditor`];
    delete sdkPrefs[`${PREFS}EnablePB`];
    delete sdkPrefs[`${PREFS}Context`];
    delete sdkPrefs[`${PREFS}ForceRemove`];
    delete sdkPrefs[`${PREFS}Panel`];
