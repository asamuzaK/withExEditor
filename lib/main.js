...
"use strict";
{
  ...
  const removeTabRelatedStorage = async data => {
    ...
    if (data) {
      ...
      bool = tabId && tmpStorage.remove({dir, tabId});
    }
    return bool || false;
  };
  ...
  const getTmpFileData = async data => {
    let store;
    if (data) {
      const {tabId, dataId: target} = data;
      const dir = !!data.incognito && TMP_FILES_PB || TMP_FILES;
      const host = data.host || LABEL;
      store = tmpStorage.get({dir, tabId, host, target});
      store && store.path && (
        store.dataId = target,
        store.timestamp = (new fs.Stats(store.path)).mtime,
        tmpStorage.set({dir, tabId, host, target}, store)
      );
    }
    return store || null;
  };
  ...
  const createTmpFile = async (data, value = "", callback = null) => {
    let name, path;
    if (data) {
      const {tabId, host} = data;
      const dir = !!data.incognito && TMP_FILES_PB || TMP_FILES;
      name = data.fileName;
      path = join(DIR_TMP, dir, tabId, host);
    }
    return name && path && createDir(path).then(() =>
      createFile(join(path, name), value, callback, data)
    ) || null;
  };
