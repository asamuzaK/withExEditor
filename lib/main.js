...
(() => {
  ...
  */
  ...
  /* simpleStorage */
  /**
  * initialize stored tmp files data on storage over quota
  ...
  const onStorageOverQuota = evt => {
    ...
    evt && evt.type === "OverQuota" && storageId && (
      ...
      simpleStorage.quotaUsage >= 0 && simpleStorage.quotaUsage < 1 ? (
        ...
        msg = new Error(_(`On${ evt.type }Error`)),
        logError(msg),
        notifyError(msg)
      )
    );
  ...
  */
  const storeTmpFile = (store, tmpFiles) => {
    store && store.target && tmpFiles && storageId && storageId[tmpFiles] && (
      storageId[tmpFiles][store.target] = store
    );
    return;
  };
  /**
  ...
  */
  const getContextMode = (res = `{ "mode": ${ VIEW_SOURCE }}`) => {
    ...
    switch(res.mode) {
      ...
      case VIEW_SELECTION:
        res.value ? (
          value = /^(?:(?:application\/(?:[\w\.\-]+\+)?|image\/[\w\.\-]+\+)x|text\/(?:ht|x))ml$/.test(type) ?
                    res.value :
                    res.value.replace(/^<(?:[^>]+:)?[^>]+?>|<\/(?:[^>]+:)?[^>]+>$/g, "").
                              replace(/&lt;/g, "<").replace(/&gt;/g, ">").
                              replace(/&amp;/g, "&"),
