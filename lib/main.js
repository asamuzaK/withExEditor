...
"use strict";
{
  ...
  const removePrivateTmpFiles = evt => {
    evt && evt.type && evt.type === LAST_PB && tmpDir && removeDir(
      ...
      storageTmp.remove({ dir: TMP_DIR_PB });
      storageTmp.set({ dir: TMP_DIR_PB }, {});
    }).catch(notifyError);
  };
  ...
  const attachTab = (res, target) => {
    ...
    res && file && tabs.activeTab.attach({
  ...
  const getTmpFile = target => {
    ...
    const file = target && storageTmp.get({ dir, id, host, target });
    file &&
      createRequest(file.uri, file.type, "UTF-8", attachTab, target).get();
  };
   ...
   * @return {void}
   */
  const createTmpFileFromResponse = (res, ...args) => {
    const tab = tabs.activeTab;
  ...
  const getSource = (url = tabs.activeTab.url,
    ...
    uri && (
      uri.scheme === "file" ?
        ...
        createRequest(url, type, charset, createTmpFileFromResponse).get()
    );
  };
  ...
  const createTmpFileByMode = (res = `{ "mode": "${ VIEW_SOURCE }" }`) => {
    ...
    switch (obj.mode) {
      case EDIT_TEXT:
        ...
        break;
      case VIEW_SELECTION:
        value && (target = getFileNameFromURI(url.path, "index")) ?
  ...
  exports.onUnload = reason => {
    ...
    panel.port.removeListener("hide",updateUI);
    button.removeListener("click", togglePanel);
    browserWindows.removeListener("activate", toggleButtonIcon);
    events.off(LAST_PB, removePrivateTmpFiles);
    simplePrefs.removeListener(`${ PREFS }File`, replacePrefValue);
    simplePrefs.removeListener(`${ PREFS }AccessKey`, replacePrefValue);
    simplePrefs.removeListener(`${ PREFS }EnablePB`, replacePrefValue);
