...
(() => {
	...
	*/
	const getFileNameFromUriPath = (path, value = LABEL) =>
		path && (
			path = /(?:.*\/)?((?:[a-zA-Z0-9\-_~!\$&'\(\)\*\+,;=:@]|%[0-9a-fA-F]{2})+)(?:\.(?:[a-zA-Z0-9\-\_~!\$&'\(\)\*\+,;=:@]|%[0-9a-fA-F]{2})+)*$/.exec(path)
		) ? path[1] : value;
	/**
	...
	const getFileExtension = type => {
		switch(true) {
			...
			case /^application\/xhtml\+xml$/.test(type):
				type = ".xhtml";
				break;
			case /^(?:application\/(?!(?:math|xht)ml\+)(?:[a-zA-Z0-9\.\-_]+\+)?|text\/)xml$/.test(type):
				type = ".xml";
				break;
	...
	*	@param {string} tmpFiles - directory name
	*/
		...
		isEnabled && filePath && fileURI && (
			...
			editorPath !== "" && all([
				...
				array => {
					...
					run && execFile(
						editorPath,
						/\S+/.test(prefsCmdArgs) ? [filePath].concat(
							prefsCmdArgs.match(/"[^"]?(?:(?:[^"]|\\"[^"]?(?:[^"]*[^"\s])?\\")*[^"\s])?"|[^"\s]+/g)
						) : [filePath],
	...
	*/
	const setTmpFile = res => {
		...
		isEnabled && res && (
			...
			componentUtils.createFile([tmpDir, tmpFiles, res.fileName], res.value, openFileWithApp)
		);
	};
	/**
		...
		if(res) {
			...
			switch(true) {
				...
				case /^mode=ViewSelection;value=.*/.test(res):
					...
					!/^(?:(?:application\/(?:[a-zA-Z0-9\.\-_]+\+)?|image\/[a-zA-Z0-9\.\-_]+\+)x|text\/(?:ht|x))ml$/.test(type) && (
	...
	*/
	const onLastPbContextExisted = () => {
		let path;
		tmpDir && storageId && (
			path = componentUtils.joinPath([tmpDir, PB_TMP_FILES_DIR_NAME]),
	...
	const setContextMenuItem = () => {
		...
		contextMenuItem = new Item({
			...
			context: contextMenu.PredicateContext(
				context =>
					isEnabled && /^(?:application\/(?:(?:[a-zA-Z0-9\.\-_]+\+)?(?:json|xml)|(?:ecm|jav)ascript)|image\/[a-zA-Z0-9\.\-_]+\+xml|text\/[a-zA-Z0-9\.\-_]+)$/.test(context.documentType)
			),
	...
	*/
	const replaceEditor = bool => {
		bool ? (
			...
			editorName = prefsFile ? getFileNameFromUriPath(urls.URL(urls.fromFilename(editorPath)).path) : ""
		) : (
			editorPath = "",
			editorName = "",
	...
	*	@param {string} k - key
	*	@returns {string} - a single character key or empty string
	*/
		...
		*/
		const setItems = bool => {
			...
			bool && prefsFile && (
				...
				editorName = stored && stored.editorPath && stored.editorPath === stored.prefsFile && stored.editorName ? stored.editorName : getFileNameFromUriPath(urls.URL(urls.fromFilename(prefsFile)).path)
			);
