...
(() => {
	...
	exports.main = (option, callback) => {
		let storageTmpFilesProcessId,
			...
			contextMenuItem,
			accessKey = "e",
			recommendMinVersion = "39.*";

		/* modules */
		function getFileNameFromUriPath(path, value) {
			let fileName = path && /(?:.*\/)?((?:[a-zA-Z0-9\-_~!\$&'\(\)\*\+,;=:@]|%[0-9a-fA-F]{2})+)(?:\.(?:[a-zA-Z0-9\-\_~!\$&'\(\)\*\+,;=:@]|%[0-9a-fA-F]{2})+)*$/.exec(path);
		...
		function getTmpFile(target) {
			...
			target && isEnabled && (
				...
				tmpFiles && storageTmpFilesProcessId && storageTmpFilesProcessId[tmpFiles] && storageTmpFilesProcessId[tmpFiles][target] && (
		...
		function openFileWithExEditor(filePath, fileURI) {
			...
			processId && isEnabled && filePath && fileURI && (
				...
				target && storageTmpFilesProcessId && storageTmpFilesProcessId[tmpFiles] && storageTmpFilesProcessId[tmpFiles][target] && (
					...
					componentUtils.openFileWithApp(filePath, simplePrefs.prefs[PREFS_FILE_NAME], notifyOnStderr)
				)
			);
		}
		...
		function getSource() {
			let activeTab = tabs.activeTab, url = activeTab.url, type = activeTab.contentType;
			return url && type && Request({
				url: url,
				contentType: type,
				onComplete(res) {
					let target = getFileNameFromUriPath(urls.URL(url).path, "index"),
		...
		function parseContextStringToVariables(res) {
			let activeTab, type, target, fileName, value;
			if(res) {
		...
		function setControlPanel() {
			...
			controlPanel.port.on("load", () => {
				workerPortEmit(controlPanel, "htmlValue", {
					...
					"compat": componentUtils.compareVersion(recommendMinVersion),
					"accessKeyPlaceholder": _("AccessKey.placeholder"),
		...
		/* private browsing */
		function notifyOnFailRemoveTmpFiles(e) {
			let reason = e.becauseExists ? _("BecauseExists") : becauseNoSuchFile ? _("BecauseNoSuchFile") : becauseClosed && _("BecauseClosed");
			notify({
				title: e.operation ? e.operation : _("Error"),
				text: reason ? _("NotifyOnFailRemoveTmpFilesBecause", reason) : _("NotifyOnFailRemoveTmpFiles")
			});
		}
		function onLastPbContextExisted() {
			let path, opt = prefsForceRemove ? { ignorePermissions: true } : null;
			processIdTmpDir && storageTmpFilesProcessId && (
				path = componentUtils.joinPath([processIdTmpDir, PB_TMP_FILES_DIR_NAME]),
				componentUtils.initDirPath(path, opt, notifyOnFailRemoveTmpFiles),
		...
		function setItems(bool) {
			let exEditor = simpleStorage.storage.withExEditor || false;
	...
	exports.onUnload = reason => {
		let path, opt = simplePrefs.prefs[PREFS_FORCE_REMOVE] ? { ignorePermissions: true } : null;
		simpleStorage.storage.withExEditorTmpFiles && processId && (
			path = simpleStorage.storage.withExEditorTmpFiles[processId],
			path && (
				path["processIdTmpDir"] && componentUtils.attemptRemoveDir(path["processIdTmpDir"], opt),
