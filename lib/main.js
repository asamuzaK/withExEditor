  ...
  class ChildProcess {
    ...
    constructor(arg) {
      ...
      this.opt = arg && arg.opt && (
                   arg.opt.cwd || arg.opt.env || arg.opt.argv0 ||
                   arg.opt.stdio || arg.opt.detached || arg.opt.shell
                 ) && arg.opt || null;
     ...
     */
    spawn(file) {
      const child = isFile(file) && isExecutable(this.app) &&
                      spawn(
                        this.app,
                        concatArg(file.replace(/\\/g, "\\\\"), this.cmd),
                        this.opt
                      ) || null;
      child && child.on('error', e => {
        this.cb && this.cb(e);
      });
      return child;
    }
