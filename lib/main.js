...
(() => {
  ...
  const FILE_EXT = JSON.parse(self.data.load("json/fileExt.json"));
  const PREF_ID = `${ (() => {
  ...
  const getFileExtension = (type = "text/plain") => {
    let ext;
    if(type = /^(application|image|text)\/([a-z0-9\-\.]+)(?:\+(json|xml))?$/i.exec(type)) {
      const mime = {
        type : type[1],
        subtype: type[2],
        suffix: type[3] ||
      ...
      ext = mime.suffix ? (
              FILE_EXT[mime.type][mime.suffix][mime.subtype] ||
              FILE_EXT[mime.type][mime.suffix][mime.suffix]
            ) : FILE_EXT[mime.type][mime.subtype];
    }
    return `.${ ext ? ext : "txt" }`;
  };
