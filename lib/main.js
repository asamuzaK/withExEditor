...
"use strict";
{
  ...
  class ChildProcess {
     ...
     * @param {Object} [arg.opt] - options
     * @param {Function} [arg.onError] - callback on child process error
     * @param {boolean} [arg.shell] - spawn shell
     */
    constructor(arg) {
      this.app = arg && isExecutable(arg.app) && arg.app || "";
      this.cmd = arg && isString(arg.cmd) && arg.cmd || "";
      this.opt = arg && arg.opt && arg.opt.env && arg.opt || null;
      this.cb = arg && typeof arg.onError === "function" && arg.onError || null;
      this.shell = arg && arg.shell || false;
    }
    ...
    spawn(file) {
      ...
      if (isFile(file) && this.app) {
        const cmd = this.shell ?
                    system.env.COMSPEC ?
                      system.env.COMSPEC :
                      "/bin/sh" :
                      this.app;
        const arg = this.shell ?
                    system.env.COMSPEC ?
                      concatArg(
                        "/c",
        ...
        proc = execFile(cmd, arg, this.opt, this.cb);
      }
      return proc;
    }
  }
