...
"use strict";
{
   ...
   */
  const getFilePathFromURI = uri =>
    isString(uri) && urls.toFilename(uri);
  ...
  const storeTmpFilePath = async (path, data) => {
    ...
    const tabId = data.tabId;
    const host = data.host || LABEL;
    const target = path && await getFileNameFromFilePath(path);
    path && dir && tabId && host && target && (
      data.path = path,
  ...
  const createTmpFile = async (data, value = "", callback = null) => {
    ...
    return dir && tabId && host && fileName &&
    ...
    webExtension.startup().then(api => {
      ...
      const portWebExtension = async port => {
        ...
        const getTmpFile = async data => {
          ...
          return path && readFile(path, portSyncText, data);
        };
        ...
        const getAppManifest = async path =>
          await isFile(path) && readFile(path, portAppManifest);
            ...
            for (let item of items) {
              ...
              switch (item) {
                ...
                case GET_APP_MANIFEST:
                  ...
                  break;
                case GET_FILE_PATH:
                  portFilePath(getFilePathFromURI(obj.uri)).catch(logError);
                  break;
                case GET_TMP_FILE:
