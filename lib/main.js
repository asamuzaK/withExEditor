...
(() => {
	...
	exports.main = function(option, callback) {
		let storageTmpFilesProcessId, tmpDir, processIdTmpDir,
			prefsFile, prefsEnablePb, prefsForceRemove, isPb, isEnabled,
			editorName = "", editorPath = "", accessKey = "e",
		...
		function storeProcessIdTmpFiles() {
			simpleStorage.storage.withExEditorTmpFiles && (
				simpleStorage.storage.withExEditorTmpFiles[PROCESS_ID] = {
				...
				storageTmpFilesProcessId = simpleStorage.storage.withExEditorTmpFiles[PROCESS_ID]
			);
		}
		...
		function toggleTab(href) {
			if(href) {
				let tab;
				for(let i = 0, l = tabs.length; i < l; i = (i + 1) | 0) {
					if(tabs[i].url === href) {
						tab = tabs[i]; break;
					}
				}
		...
		function onLastPbContextExisted() {
			...
			processIdTmpDir && storageTmpFilesProcessId && (
				...
				componentUtils.initDirPath(path, prefsForceRemove ? { ignorePermissions: true } : null, notifyOnFailRemoveTmpFiles),
