...
"use strict";
{
  ...
  class StorageTmp {
    ...
    get(des) {
      ...
      return dir && tId && host && target &&
             store.storage[dir] && store.storage[dir][tId] &&
             store.storage[dir][tId][host] &&
               store.storage[dir][tId][host][target];
    }
    ...
    set(des = {}, val = null) {
      ...
      dir && (store.storage[dir] = store.storage[dir] || tId && {} || val) &&
      tId && (store.storage[dir][tId] = store.storage[dir][tId] ||
                                        host && {} || val) &&
      host && (store.storage[dir][tId][host] = store.storage[dir][tId][host] ||
                                               target && {} || val) &&
      target && (store.storage[dir][tId][host][target] = val);
    }
    ...
    remove(des = {}) {
      ...
      const host = isString(des.host) && des.host;
      const target = isString(des.target) && des.target;
      return dir && !tId && store.storage[dir] ?
               delete internal(this).storage[dir] :
             dir && tId && !host &&
             store.storage[dir] && store.storage[dir][tId] ?
               delete internal(this).storage[dir][tId] :
             dir && tId && host && !target && store.storage[dir] &&
             store.storage[dir][tId] && store.storage[dir][tId][host] ?
               delete internal(this).storage[dir][tId][host] :
             dir && tId && host && target && store.storage[dir] &&
             store.storage[dir][tId] && store.storage[dir][tId][host] &&
             store.storage[dir][tId][host][target] &&
               delete internal(this).storage[dir][tId][host][target];
    }
  }
  ...
  const attachTab = (res, target) => {
    ...
    const file = target && storageTmp.get({dir, tId, host, target});
    file && file.path && (
