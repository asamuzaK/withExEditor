...
"use strict";
{
  ...
  /* data */
  const fileExt = JSON.parse(self.data.load("../webextension/data/fileExt.json"));
  const sysEnv = JSON.parse(self.data.load("../webextension/data/sysEnv.json"));
  const nsURI = self.data.load("../webextension/data/nsUri.json");
  ...
  /**
   * set temporary file data to storage
   * @param {Object} data - file data
   * @return {Object}
   */
  const setTmpFileData = async data => {
   ...
   * @param {string} obj - content data
   * @return {Object|undefined}
   */
  const createTmpFileByMode = async obj => {
    const mode = obj.mode;
      ...
      webExtension.startup().then(api => {
        ...
        const portWebExtension = async port => {
          ...
          const portSyncText = async (res, data) => {
            const file = await setTmpFileData(data);
            file && portMsg({
              syncText: {
                dataId: file.target,
                tabId: file.tabId,
          ...
          /**
           * get temporary file
           * @param {Object} data - temporary file storage data
           * @return {Object} - Request
           */
          const getTmpFile = async data =>
            createRequest(data.uri, data.type, CHAR, portSyncText, data).get();
