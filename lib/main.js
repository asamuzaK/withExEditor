...
"use strict";
{
  ...
  const TMP_DIR = paths.join(
    system.pathFor("TmpD"),
    LABEL,
    /^.+\.([^\.]+)$/.test(paths.basename(PREF_D)) &&
      /^.+\.([^\.]+)$/.exec(paths.basename(PREF_D))[1] || LABEL
  );
  ...
  const getFileNameFromFilePath = path => {
    const file = isString(path) &&
                   /^([^\.]+)(?:\..+)?$/.exec(paths.basename(path));
   ...
   * @param {string} [arg.path] - file path
   * @param {string} [arg.value] - value to write
   * @param {string} [arg.encoding] - file encoding
   * @param {Function} [arg.cb] - callback when write completes
   * @return {void}
   */
  const createFile = (arg = {}) => {
    arg && isString(arg.path) && typeof arg.cb === "function" &&
      fs.writeFile(arg.path, arg.value || "", arg.encoding || CHAR, arg.cb);
  };
  ...
  const removePrivateTmpFiles = evt =>
    evt && evt.type === LAST_PB &&
      removeDir(
        paths.join(TMP_DIR, TMP_FILES_PB),
  ...
  class ChildProcess {
     ...
     */
    constructor(arg = {}) {
      this.app = isExecutable(arg.app) && arg.app || "";
  ...
  const spawnProcess = path => {
    ...
    return path && editorPath && tab.once("close", removeTabRelatedStorage) &&
             (new ChildProcess({
               ...
               env: getSystemEnv(),
               pos: prefs[`${PREFS}CmdPos`],
               shell: prefs[`${PREFS}Shell`]
  ...
  const createTmpFile = (data = {}, value = "") => {
    ...
    return tId && host && target && file &&
             createDir(paths.join(TMP_DIR, dir, tId, host)).then(path => {
               storage.set({dir, tId, host, target}, data);
               createFile({
                 path: paths.join(path, file),
                 value,
                 cb: e => {
                   e ?
                     logError(e) :
                     spawnProcess(paths.join(path, file));
                 }
               });
             }).catch(logError);
  };
  ...
  const createHotkey = (key, callback) =>
    isString(key) && key.length === 1 && typeof callback === "function" &&
      new Hotkey({
        combo: `accel-alt-${key}`,
        onPress: callback
      }) || null;
  ...
  class AccessKey {
    ...
    constructor(opt = {}) {
      ...
      this._hotkey = opt.hotkey && opt.onPress &&
                       createHotkey(this._key, opt.onPress) || null;
     ...
     * @return {void}
     */
    setHotkey(enabled, callback) {
      this._hotkey && this._hotkey.destroy();
      this._hotkey = enabled && callback &&
  ...
  const getToolbarButtonLabel = (key, hotkey) => {
    ...
    return isString(key) && key.length === 1 && hotkey &&
  ...
  const replaceEditor = pref => {
    ...
    if (pref === `${PREFS}File`) {
      ...
      try {
        pref === `${PREFS}Path` && isExecutable(path) &&
          (prefs[`${PREFS}File`] = path);
      }
      catch (e) {
        return;
      }
  ...
  const replaceHotkey = pref => {
    ...
    accKey.setHotkey(hotkey, togglePanel);
  ...
  const replaceAccessKey = pref => {
    ...
    replaceHotkey(`${PREFS}Hotkey`);
    replacePageMod(`${PREFS}ExecEditor`);
  ...
  exports.main = () => {
    ...
    const path = prefs[`${PREFS}File`] || prefs[`${PREFS}Path`];
    ...
    simplePrefs.on(`${PREFS}Path`, replaceEditor);
    ...
    all([
      createDir(paths.join(TMP_DIR, TMP_FILES)),
      createDir(paths.join(TMP_DIR, TMP_FILES_PB)),
      setToolbarButton(),
      setPanelEvent(),
  ...
  exports.onUnload = reason => {
    ...
    reason === "uninstall" && (
      ...
      removeDir(paths.join(PREF_D, "jetpack", self.id), true).catch(logError)
    );
  };
}
