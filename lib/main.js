...
"use strict";
{
  ...
  const getFileNameFromFilePath = path => {
    const file = /^([^\.]+)(?:\..+)?$/.exec(paths.basename(path));
   ...
   * @return {void}
   */
  const createFile = (path, value = "", encoding = CHAR, callback = null) => {
    fs.writeFile(path, value, encoding, callback || logError);
  };
  ...
  const replaceEditor = pref => {
    ...
    if (pref === `${PREFS}File`) {
      isString(path) && isExecutable(path) ? (
        editorPath = path,
      ...
      try {
        pref === `${PREFS}Path` && isString(path) && isExecutable(path) &&
