...
"use strict";
{
  ...
  const concatArg = (...args) => {
    ...
    const arr = args.map(arg => {
      isString(arg) && (arg = arg.match(EXP_CMD));
    ...
    setPageMod() {
      this._key ? (
        pageModWorkers.forEach(worker => {
          worker.port.emit("detach");
          worker.destroy();
        }),
        this._page.destroy(),
        this._page = createPageMod({
          key: this._key,
          ctrlKey: true,
          altKey: false,
          shiftKey: true,
  ...
  const mediaTypeContext = new contextMenu.PredicateContext(context =>
    EXP_MEDIA_TYPE.test(context.documentType)
  );
