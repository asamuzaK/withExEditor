...
"use strict";
{
  ...
  class ChildProcess {
    ...
    spawn(file) {
      /**
       * remove quotes from argument string
       * @param {string} arg - argument
       * @return {string} - argument
       */
      const removeQuotes = arg => {
        const quotedArg = /^"(.*)"$/.exec(arg) || /^'(.*)'$/.exec(arg);
        return quotedArg && quotedArg[1] || arg;
      };

      /**
       * command line arguments to an array
       * @param {string} path - file path
       * @param {string} cmd - command line arguments
       * @return {Array} - arguments array
       */
      const argsToArray = (path, cmd) => {
        const args = cmd.match(EXP_CMD);
        return args && [path].concat(args.map(removeQuotes)) || [path];
      };

      return isFile(file) && isExecutable(this.app) &&
               execFile(
                 this.app,
                 argsToArray(file, this.cmd),
                 this.opt,
                 this.callback
               );
    }
