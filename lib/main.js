...
(() => {
	...
	const PROCESS_ID = (() => {
		return `${ LABEL }_${ componentUtils.getProfName() || new Date().now() }`;
	})();
	...
	exports.main = function(option, callback) {
		...
		function notifyOnError(e, v = null) {
			let title = _("Error"), text = e && e.operation ? e.operation : e && e.message ? e.message : e.toString();
			if(e && e.operation && v) {
				let reason = e.becauseExists ? _("BecauseExists") : e.becauseNoSuchFile ? _("BecauseNoSuchFile") : e.becauseClosed && _("BecauseClosed");
				title = e.operation;
		...
		function openFileWithExEditor(filePath, fileURI) {
			...
			isEnabled && filePath && fileURI && (
				...
				storageTmpFilesProcessId && storageTmpFilesProcessId[tmpFiles] && storageTmpFilesProcessId[tmpFiles][target] && (
					...
					componentUtils.openFileWithApp(filePath, simplePrefs.prefs[PREFS_FILE_NAME], notifyOnError)
				)
			);
		}
		...
		function parseContextStringToVariables(res) {
			let activeTab, type, url, target, fileName, value;
			if(res) {
