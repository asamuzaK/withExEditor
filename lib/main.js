...
(() => {
   ...
   */
  const getFileNameFromBasename = path =>
    path && (
      path = /^([^\.]+)(?:\..+)?$/.exec(paths.basename(path))
    ) ? path[1] : LABEL;
  ...
  const getTmpFile = target => {
    if(isEnabled && target) {
      ...
      file && createRequest(file.fileURI, file.type, "UTF-8", res => {
        if(res) {
   ...
   * @param {string} path - file path
   */
  const execEditorParams = (path) => {
    if(isEnabled && path && editorPath) {
      const dirName = isPb ? TMP_DIR_PB : TMP_DIR;
      const target = getFileNameFromBasename(path);
      storeId && storeId[dirName] && storeId[dirName][target] &&
        (storeId[dirName][target].fileURI = urls.fromFilename(path));
      openFileWithApp(
        path,
        editorPath,
        prefsArgs || "",
  ...
  const getSource = (url = tabs.activeTab.url,
    ...
    if(isEnabled && urls.isValidURI(url)) {
      ...
      if(uri.protocol === "file:") {
        ...
        execEditorParams(urls.toFilename(url));
      }
      else {
        createRequest(url, type, charset, res => {
          if(res) {
   ...
   * @return {Object} - Promise object, return boolean on fulfill
   */
  const setTmpDir = () =>
  ...
  const replaceEditor = bool => {
    bool && prefsFile ? (
      ...
      editorName = getFileNameFromBasename(prefsFile)
    ) : (
      editorPath = "",
      editorName = "",
      menuItem = null,
    ...
    const setItems = (ex = false) => {
      ...
      ex && prefsFile && (
        ...
        editorName = stored && stored.editorPath &&
                     ...
                     stored.editorName ?
                       stored.editorName :
                       getFileNameFromBasename(prefsFile)
      );
