...
(() => {
  ...
  const fileIsExecutable = path =>
    fileIsFile(path) &&
      Boolean((new fs.Stats(path)).mode & parseInt("0001", 8));
   ...
   */
  const setTmpDir = () =>
    createDir(paths.join(system.pathFor("TmpD"), LABEL)).then(path =>
      createDir(paths.join(path, PREF_ID))
    ).then(path =>
      (tmpDir = path) && all([
        createDir(paths.join(tmpDir, TMP_DIR)),
        createDir(paths.join(tmpDir, TMP_DIR_PB))
      ])
    ).then(arr => {
      let bool = false;
      if(arr) {
  ...
  exports.main = (option, callback) => {
    ...
    setItems(prefsFile ? fileIsExecutable(prefsFile) : false);
  };
  ...
  exports.onUnload = reason => {
    ...
    storeId && storeId.tmpDir && (
      removeDir(
        storeId.tmpDir,
        simplePrefs.prefs[`${ PREFS }ForceRemove`] ? true : false
      ).then(() => {}, () => {}),
