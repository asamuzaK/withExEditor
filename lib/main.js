...
"use strict";
{
   ...
   * @return {boolean}
   */
  const isEnabled = () => !isPb() || isPb() && prefs[`${ PREFS }EnablePB`];
  ...
  const onTmpFileRes = (res, target) => {
    ...
    const host = new urls.URL(tabs.activeTab.url).host || LABEL;
    const file = target && storageTmp[dir] && storageTmp[dir][host] &&
                   storageTmp[dir][host][target];
  ...
  const createTmpFile = (res = {}, value = "") => {
    ...
    host && target && file && (
      storageTmp[dir] &&
      (storageTmp[dir][host] = storageTmp[dir][host] || {}) &&
        (storageTmp[dir][host][target] = res),
      createDir(files.join(tmpDir, dir, host)).then(path => {
  ...
  const onSourceRes = (res, ...args) => {
    ...
    const url = tab.url;
    const host = new urls.URL(url).host || LABEL;
    const target = getFileNameFromURI((new urls.URL(url)).path, "index");
  ...
  const switchByMode = (res = `{ "mode": "${ VIEW_SOURCE }" }`) => {
    ...
    switch (obj.mode) {
      ...
      case VIEW_SELECTION:
        value ? (
          target = getFileNameFromURI(new urls.URL(url).path, "index"),
