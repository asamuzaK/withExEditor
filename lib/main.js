...
(() => {
  ...
  const storeKeyValue = (key, value = "") => {
    if(key) {
      simpleStorage.storage.withExEditor[key] = value;
    }
  };
  ...
  const onLastPbContextExited = evt => {
    if(evt && evt.type && evt.type === LAST_PB && tmpDir && storeId) {
  ...
  exports.onUnload = reason => {
    ...
    simplePrefs.prefs[`${ PREFS }EnvDisplay`] && (
      delete simplePrefs.prefs[`${ PREFS }EnvDisplay`]
    );
    simplePrefs.prefs[`${ PREFS }OnDisable`] && (
      delete simplePrefs.prefs[`${ PREFS }OnDisable`]
    );
