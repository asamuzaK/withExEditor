...
(() => {
  ...
  const getFileExtension = (type = "text/plain") => {
    switch(true) {
      ...
      case /^application\/(?!ld\+)(?:[\w\.\-]+\+)?json$/.test(type):
        type = "json";
        break;
      case /^application\/sparql-query$/.test(type):
        type = "rq";
        break;
      case /^application\/emma\+xml$/.test(type):
        type = "emma";
        break;
      case /^application\/emotionml\+xml$/.test(type):
        type = "emotionml";
        break;
      case /^application\/its\+xml$/.test(type):
        type = "its";
        break;
      case /^application\/mathml\+xml$/.test(type):
        type = "mml";
        break;
      case /^application\/provenance\+xml$/.test(type):
        type = "provx";
        break;
      case /^application\/sparql-results\+xml$/.test(type):
        type = "srx";
        break;
      case /^application\/ttml\+xml$/.test(type):
        type = "ttml";
        break;
      case /^application\/wsdl\+xml$/.test(type):
        type = "wsdl";
        break;
      case /^application\/wspolicy\+xml$/.test(type):
        type = "wspolicy";
        break;
      case /^application\/xhtml\+xml$/.test(type):
        type = "xhtml";
        break;
      case /^application\/xslt\+xml$/.test(type):
        type = "xsl";
        break;
      case /^(?:application\/(?!(?:(?:emotion|math|tt|xht)ml|(?:it|sparql-result)s|ws(?:policy|dl)|emma|provenance|xslt)\+)(?:[\w\.\-]+\+)?|text\/)xml$/.test(type):
        type = "xml";
        break;
      ...
      case /^text\/n3$/.test(type):
        type = "n3";
        break;
      case /^text\/provenance-notation$/.test(type):
        type = "provn";
        break;
   ...
   */
  const initDir = (path, rec = false, callback = null) =>
    removeDir(path, rec).then(path =>
      createDir(path)
    ).catch(e => {
      callback && callback(e);
      logError(e);
   ...
   */
  const syncContentsValue = (target = "", value = "", ts = 0, ns = null) => {
  ...
  const onTmpFileRes = (res, file) => {
    if(res && file) {
      ...
      const mtime = (new fs.Stats(file.path)).mtime;
      file.value = value;
      file.mtime = mtime;
      storeTmpFile(file, isPb ? TMP_DIR_PB : TMP_DIR);
      syncContentsValue(file.target, value, mtime, file.namespace || null);
    }
  };
  ...
  const setOpenFileWithAppParams = path => {
    if(isEnabled && path && editorPath) {
      ...
      storeId && storeId[dirName] && storeId[dirName][target] && (
        storeId[dirName][target].path = path,
  ...
  const onLastPbContextExited = evt => {
    evt && evt.type && evt.type === LAST_PB && tmpDir && storeId && (
      initDir(files.join(tmpDir, TMP_DIR_PB), prefsRemove, notifyError),
      storeId.tmpFilesPb = {}
    );
  };
  ...
  exports.main = (option, callback) => {
    ...
    prefsFile && fileIsExecutable(prefsFile) && (
      store && storeKeyValue("prefsFile", prefsFile),
      editorPath = prefsFile,
    ...
    store && store.toolbarButtonIcon && (
      buttonIcon = store.toolbarButtonIcon
    );
    storeSettings({
      prefsFile: prefsFile,
      editorName: editorName,
    ...
    createDir(files.join(system.pathFor("TmpD"), LABEL, PREF_ID)).then(path =>
      (tmpDir = path) && all([
        createDir(files.join(tmpDir, TMP_DIR)),
        createDir(files.join(tmpDir, TMP_DIR_PB)),
        setToolbarButton(store ? true : false),
        setContextMenuItem(),
        setPanel()
      ])
    ).then(arr => {
  ...
  exports.onUnload = reason => {
    ...
    removeDir(tmpDir, prefsRemove).then(() => {}, () => {});
